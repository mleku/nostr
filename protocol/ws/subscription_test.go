...
func TestSubscribeBasic(t *testing.T) {
	...
	defer rl.Close()
	sub, err := rl.Subscribe(context.Background(), filters.New(&filter.T{Kinds: kinds.New(kind.TextNote), Limit: 2}))
	if err != nil {
...
func TestNestedSubscriptions(t *testing.T) {
	...
	sub, err := rl.Subscribe(context.Background(),
		filters.New(&filter.T{
			...
			Tags:  tags.New(tag.New("e", "0e34a74f8547e3b95d52a2543719b109fd0312aba144e2ef95cba043f42fe8c5")),
			Limit: 3,
		}))
	if err != nil {
	...
	for {
		...
		case event := <-sub.Events:
			...
			sub, err := rl.Subscribe(context.Background(), filters.New(&filter.T{Kinds: kinds.New(kind.ProfileMetadata), Authors: tag.New(event.PubKey), Limit: 1}))
			if err != nil {
