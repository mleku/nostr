...
type SimplePool struct {
	...
	Context         Ctx
	authHandler     func() crypto.Signer
	cancel          context.F
...
type WithAuthHandler func() crypto.Signer
...
func (pool *SimplePool) subMany(c Ctx, urls []S, ff *filters.T, unique bool) chan IncomingEvent {
	ctx, cancel := context.Cancel(c)
			...
			subscribe:
				...
				for {
					...
					case evt, more := <-sub.Events:
						...
						if unique {
							if _, seen := seenAlready.LoadOrStore(evt.EventID().String(), *evt.CreatedAt); seen {
								continue
							}
						}
						select {
...
func (pool *SimplePool) subManyEose(c Ctx, urls []S, ff *filters.T, unique bool) chan IncomingEvent {
	ctx, cancel := context.Cancel(c)
		...
		subscribe:
			...
			for {
				...
				case evt, more := <-sub.Events:
					...
					if unique {
						if _, seen := seenAlready.LoadOrStore(evt.EventID().String(), true); seen {
							continue
						}
					}
