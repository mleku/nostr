...
func NewRelay(c context.T, url S, opts ...RelayOption) *Relay {
...
func (r *Relay) Connect(c context.T) error { return r.ConnectWithTLS(c, nil) }
...
func (r *Relay) Publish(c context.T, ev *event.T) E { return r.publish(c, ev) }

// Auth sends an "AUTH" command client->relay as in NIP-42 and waits for an OK response.
func (r *Relay) Auth(c context.T, signer nostr.Signer) error {
...
func (r *Relay) publish(ctx context.T, ev *event.T) (err E) {
	var cancel context.F
	if _, ok := ctx.Deadline(); !ok {
		// if no timeout is set, force it to 7 seconds
...
func (r *Relay) Subscribe(c context.T, ff *filters.T,
...
// Remember to cancel subscriptions, either by calling `.Unsub()` on them or ensuring their `context.Context` will be canceled at some point.
// Failure to do that will result in a huge number of halted goroutines being created.
func (r *Relay) PrepareSubscription(c context.T, ff *filters.T,
...
func (r *Relay) Count(c context.T, ff *filters.T, opts ...SubscriptionOption) (int, error) {
	sub := r.PrepareSubscription(c, ff, opts...)
